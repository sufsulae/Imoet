...
namespace Imoet
{
    ...
    {
        private List<CurvePoint<Point2>> m_points;

        public int pointCount {
            get { return m_points.Count; }
        }
        public CurvePoint<Point2> this[int idx] {
            get { return GetPoint(idx); }
            set { SetPoint(idx, value); }
        }

        public Curve2D() {
            m_points = new List<CurvePoint<Point2>>();
        }
        public void AddPoint(int idx, Point2 point) {
            var newPoint = new CurvePoint<Point2>();
            newPoint.point = newPoint.handleA = newPoint.handleB = point;
            m_points.Insert(idx, newPoint);
        }
        public void AddPoint(int idx, CurvePoint<Point2> point) {
        ...
        public void AddPoint(CurvePoint<Point2> point) {
            m_points.Add(point);
        }
        public void RemovePoint(int idx) {
            m_points.RemoveAt(idx);
        }
        public CurvePoint<Point2> GetPoint(int idx) {
            return m_points[idx];
        }
        public void SetPoint(int idx, CurvePoint<Point2> point) {
            m_points[idx] = point;
        }

        public Point2 Evaluate(float amount, CurveType type) {
            if (m_points.Count < 2)
