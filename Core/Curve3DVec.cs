...
namespace Imoet
{
    ...
    [Serializable]
    public class Curve3DVec
    {
        private List<CurvePoint<Vec3>> m_points;

        public int pointCount {
            get { return m_points.Count; }
        }

        public CurvePoint<Vec3> this[int idx]
        {
        ...
        public Curve3DVec() {
            m_points = new List<CurvePoint<Vec3>>();
        }

        public void AddPoint(int idx, Vec3 point)
        {
            var newPoint = new CurvePoint<Vec3>();
            newPoint.point = newPoint.handleA = newPoint.handleB = point;
            m_points.Insert(idx, newPoint);
        }
        public void AddPoint(int idx, CurvePoint<Vec3> point)
        {
        ...
        public void AddPoint(CurvePoint<Vec3> point)
        {
            m_points.Add(point);
        }
        ...
        public CurvePoint<Vec3> GetPoint(int idx)
        {
            return m_points[idx];
        }
        public void SetPoint(int idx, CurvePoint<Vec3> point)
        {
            m_points[idx] = point;
        }
