...
namespace Imoet
{
    ...
    {
        private List<CurvePoint<Point3>> m_points;

        public int pointCount {
            get { return m_points.Count; }
        }

        public CurvePoint<Point3> this[int idx]
        {
        ...
        public Curve3D() {
            m_points = new List<CurvePoint<Point3>>();
        }
        public void AddPoint(int idx, Point3 point) {
            var newPoint = new CurvePoint<Point3>();
            newPoint.point = newPoint.handleA = newPoint.handleB = point;
            m_points.Insert(idx, newPoint);
        }
        public void AddPoint(int idx, CurvePoint<Point3> point) {
        ...
        public void AddPoint(CurvePoint<Point3> point) {
            m_points.Add(point);
        }
        public void RemovePoint(int idx) {
            m_points.RemoveAt(idx);
        }
        public CurvePoint<Point3> GetPoint(int idx) {
            return m_points[idx];
        }
        public void SetPoint(int idx, CurvePoint<Point3> point) {
            m_points[idx] = point;
        }

        public Point3 Evaluate(float amount, CurveType type) {
            if (m_points.Count < 2)
